...
#the upper and lower limits for the hsv values for each color has been obtained from the net
#masking the image with all the three colors and checking each one of them
...
count_red = 0
count_yellow = 0
#On masking an image, the output mask contains only two pixel values,255 if within the limit and 0 otherwise
#so the mask which gives a value of 255 is the right mask which gives the required color 
